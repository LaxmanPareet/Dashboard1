...
if not os.path.exists(path):
    ...
    st.info(f"Available files in directory: {[f for f in os.listdir('.') if f.endswith('.xlsx')]}")
    st.stop()
...
if start_dt and end_dt and date_col:
    try:
        # Convert to datetime only for non-null values to save memory
        date_series = filtered[date_col].dropna()
        if len(date_series) > 0:
            # Convert only non-null dates
            date_series = pd.to_datetime(date_series, errors="coerce")
            # Create mask for valid dates within range
            date_mask = date_series.dt.date.between(start_dt, end_dt)
            # Apply mask to original filtered dataframe
            valid_indices = date_series[date_mask].index
            filtered = filtered.loc[valid_indices]
    except Exception as e:
...
fail_rows = len(df) - success_rows  # from all data

# Show debugging info
with st.expander("Debug Information", expanded=False):
    st.write(f"**Original data rows:** {len(df):,}")
    st.write(f"**After filtering:** {total_rows:,}")
    if selected_module:
